...
namespace ResourceCatalog.Api.Controllers
{
    ...
    public class ResourcesController(IMediator mediator) : ControllerBase
    {
        ...
        {
            var result = await mediator.Send(new ListResource.Query(), ct);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> Get(Guid id, CancellationToken ct)
        {
            ...
            return await mediator.Send(new GetResource.Query(id), ct) switch
            {
        ...
        public async Task<IActionResult> Create([FromBody] CreateResourceRequest body, CancellationToken ct)
        {
            ...
            var result = await mediator.Send(new CreateResource.Command(body.Name, body.Description), ct);
            return CreatedAtAction(nameof(Get), new { id = result.Id }, result);
        }

        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdateResourceRequest body, CancellationToken ct)
        {
            return await mediator.Send(new UpdateResource.Command(id, body.Name, body.Description), ct) switch
            {
        ...
        {
            return await mediator.Send(new DeleteResource.Command(id), ct) switch
            {
